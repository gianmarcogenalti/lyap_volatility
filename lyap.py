...
def jacobian_mat(q, x, alpha_1, beta_0, beta_1, act_function):
    J = np.zeros((m,m))
...
def lyapunov_coeff(q, x, alpha_0, alpha_1, beta_0, beta_1, act_function):
    M=int(T**(2/3))
    Tm=np.identity(m)
    for i in range(M,10,-1):
        xm = x[(t-1):-1:(t-m)]
        J=jacobian_mat(q, xm, alpha_1, beta_0, beta_1, act_function)
        Tm=np.dot(Tm,J)
    U0=np.zeros((m,1))
    U0[0]=1
    Lx=np.dot(Tm,U0)
